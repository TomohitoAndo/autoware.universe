...
public:
    ...
    const PathWithLaneId & path, const PathWithLaneId & smoothed_path)
  {
    ...
    if (planner_param.run_out.detection_method == "Points") {
      dynamic_obstacle_data_.detection_area_low_jerk =
        createDetectionAreaPolygon(smoothed_path, planner_data, planner_param);
      dynamic_obstacle_data_.detection_area_high_jerk =
        createMandatoryDetectionAreaPolygon(smoothed_path, planner_data, planner_param);

      for (const auto & poly : dynamic_obstacle_data_.detection_area_low_jerk) {
        debug_ptr_->pushDetectionAreaPolygons(poly);
      }
      for (const auto & poly : dynamic_obstacle_data_.detection_area_high_jerk) {
        debug_ptr_->pushDetectionAreaPolygons(poly);
      }
    }
  }
...
private:
  ...
  // debug publisher
  rclcpp::Publisher<PointCloud2>::SharedPtr pub_filtered_compare_map_points_;
  rclcpp::Publisher<PointCloud2>::SharedPtr pub_filtered_vector_map_points_;
  rclcpp::Publisher<PointCloud2>::SharedPtr pub_concat_points_;
  rclcpp::Publisher<PointCloud2>::SharedPtr pub_concat_no_overlap_points_;
